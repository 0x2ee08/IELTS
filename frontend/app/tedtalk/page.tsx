...
const TedTalkPage: React.FC = () => {
    ...
    const fetchVideos = async () => {
        ...
        try {
            setLoading(true);
            const response = await axios.get(`${config.API_BASE_URL}api/get_videos`, {
                headers: {
    ...
    const takeVideos = async () => {
        const token = localStorage.getItem('token'); // Assume user is authenticated and token is available
        try {
            const response = await fetch(`${config.API_BASE_URL}api/fetch_and_save_videos`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({  }), // Pass the updated text
            });
        } catch (error) {
            console.error('Error fetching videos:', error);
        }
                        ...
                        {videos.map((video, index) => (
                            <div key={index} className="flex items-start space-x-4 border-b pb-4">
                                ...
                                <div className="flex-1">
                                    ...
                                    {/* Dislike button hidden as per your requirement */}
                                </div>
                            </div>
                        ))}
                    </div>
