...
const WritingRender: React.FC = () => {
    ...
    const getRandomStatement = async () => {
        setGenerating(true);
        const message = `Generate for me a Writing Task 2 problem without bold, highlighted text, or special characters, no yapping`;
    ...
    const handleCheckAndCorrectGrammar = async (userInput: string, apiKey: string) => {
        ...
        try {
            // Step 1: Request grammar correction from the API
            const correctionMessage = `
            Correct my essay by generate for me an essay that follow these properties:\n
            check word seperately, if the word is wrong then it will be (wrong_word)[correct_word]\n
            for example: a b c -> a (b)[correct_b] (c)[correct_c] because a is correct
            \n\n
            Here is my essay:
            \n\n
            ${userInput}
            ...
            const formatCorrections = (original: string, corrected: string) => {
                // Regex to detect patterns like (wrong text) [correction text]
                const regex = /\(([^)]+)\)\s?\[([^\]]+)\]/g;
                let match;
                ...
                while ((match = regex.exec(corrected)) !== null) {
                    const [fullMatch, wrongPhrase, correctPhrase] = match;
    
                    // Create a regular expression to match the wrong phrase in the original text
                    const wrongRegex = new RegExp(`\\b${wrongPhrase}\\b`, 'g');
                    
                    // Replace it with a span that highlights the error and shows the correction in a tooltip
                    formattedText = formattedText.replace(wrongRegex, (match) => 
                        `<span class="error" title="${correctPhrase}">${match}</span>`
                    );
                }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', backgroundColor: '#ffffff', padding: '20px', borderRadius: '10px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)', width: '100vw', height: '100vh', margin: '0', fontFamily: 'Arial, sans-serif', color: '#333' }}>
            {!isTextAreaVisible && (
                <button
                    ...
                    style={{ backgroundColor: '#4CAF50', color: 'white', padding: '12px 24px', borderRadius: '6px', border: 'none', cursor: isGenerating ? 'not-allowed' : 'pointer', width: '100%', fontSize: '16px', marginBottom: '20px' }}
                >
                    {isGenerating ? 'Generating...' : 'Generate Writing Task'}
                </button>
            )}
