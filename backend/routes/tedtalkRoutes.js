...
const fetchAllVideoDetails = async (videoIds) => {
    ...
    for (let i = 0; i < videoIds.length; i += chunkSize) {
        const videoIdChunk = videoIds.slice(i, i + chunkSize);
...
const fetchVideoIdsFromYouTube = async () => {
    const videoIds = [];
    ...
    // do {
        ...
        videoIds.push(...items.map(item => item.id.videoId));
...
const fetchAndStoreVideos = async () => {
    try {
        const videoIds = await fetchVideoIdsFromYouTube();
        const videoDetails = await fetchAllVideoDetails(videoIds);
        return videoDetails;
    } catch (error) {
...
router.get('/get_ted_videos', authenticateToken, async (req, res) => {
    try {
        ...
        if (timeDifference >= 60 * 60 * 1000) {
            console.log("Fetching new video IDs from Google API...");
            const videoIds = await fetchVideoIdsFromYouTube();
        ...
        console.error('Error fetching TED videos:', error.message);
        res.status(500).json({ message: 'Error fetching TED videos' });
    }
});
